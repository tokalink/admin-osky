...
class AdminProposalsController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		$this->col[] = ["label" => "No Proposal", "name" => "no_proposal"];
		$this->col[] = ["label" => "Status", "name" => "status"];
		// id
		$this->col[] = ["label" => "id", "name" => "id",'visible' => false];
		$this->col[] = ["label" => "Total", "name" => "id","callback" => function($row) {
			$proposal_details = DB::table('proposal_details')->where('proposal_id', $row->id)->get();
			$total = 0;
			foreach ($proposal_details as $key => $value) {
		...
		$this->form = [];
		$this->form[] = ['label' => 'Instansi', 'name' => 'instansi', 'type' => 'select', 'validation' => 'required|min:1|max:255', 'width' => 'col-sm-10', 'dataenum' => 'Perorangan;PT;Vendor', 'placeholder' => 'Please select one', 'required' => true];
		...
		$col[] = ['label' => 'Qty', 'name' => 'qty', 'type' => 'number', 'validation' => 'required|integer', 'width' => 'col-sm-10'];
		$col[] = ['label' => 'Harga', 'name' => 'harga', 'type' => 'number', 'validation' => 'required|numeric', 'width' => 'col-sm-10'];
		// $col[] = ['label' => 'Diskon', 'name' => 'diskon', 'type' => 'number', 'validation' => 'required|numeric', 'width' => 'col-sm-10'];
		// $col[] = ['label' => 'PPN', 'name' => 'ppn', 'type' => 'number', 'validation' => 'required|numeric', 'width' => 'col-sm-10'];
		// $col[] = ['label' => 'Total', 'name' => 'total', 'type' => 'number', 'validation' => 'required|numeric', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Proposal Details', 'name' => 'proposal_details', 'type' => 'child', 'columns' => $col, 'table' => 'proposal_details', 'foreign_key' => 'proposal_id', 'parent_columns' => 'instansi,name,email,phone,date,no_proposal,status,remarks,syarat', 'numbering' => true, 'width' => 'col-sm-10'];
		...
		//$this->form[] = ["label"=>"Email","name"=>"email","type"=>"email","required"=>TRUE,"validation"=>"required|min:1|max:255|email|unique:proposals","placeholder"=>"Please enter a valid email address"];
		...
		//$this->form[] = ["label"=>"No Proposal","name"=>"no_proposal","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
		...
		// link open new tab proposal pdf
		$this->addaction[] = ['label' => 'Proposal PDF', 'url' => CRUDBooster::mainpath('[id]'), 'icon' => 'fa fa-file-pdf-o', 'color' => 'success',"target" => "_blank"];
	...
	// proposal pdf
	public function getProposalPdf($id)
	{
		$proposal = DB::table('proposals')->where('id', $id)->first();
		$proposal_details = DB::table('proposal_details')->where('proposal_id', $id)->get();
		$pdf = \PDF::loadView('proposal.pdf', compact('proposal', 'proposal_details'));
